...
describe("redirect handling", () => {
  it("2>/dev/null — exact bug repro with pipeline", async () => {
    const result = await rewriteBash(
      `grep -r "foo.bar" /some/path --include="*.proto" -l 2>/dev/null | head -20`,
    );
    expect(result.rewritten).toEqual('rg "foo.bar" /some/path -g "*.proto" -l 2>/dev/null | head -20');
  });

  it("2>/dev/null — real-world bug case from user", async () => {
    const result = await rewriteBash(
      'grep -r "Not a valid ref" /var/home/l/.bun/ 2>/dev/null | head -5',
    );
    expect(result.rewritten).toEqual(
      'rg "Not a valid ref" /var/home/l/.bun/ 2>/dev/null | head -5',
    );
